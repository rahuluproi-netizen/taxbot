...
/**
 * Upserts multiple vectors with internal batching (max 100 per call) into Pinecone.
 * Reduces network roundtrips and optimizes ingestion throughput.
 * @param {Array<{id: string, values: number[], metadata: object}>} vectors - The array of vectors to upsert.
 */
async function upsertVectors(vectors) {
  ...
  try {
    ...
    for (let i = 0; i < vectors.length; i += BATCH_LIMIT) {
      const batch = vectors.slice(i, i + BATCH_LIMIT);
