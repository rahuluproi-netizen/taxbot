 ...
 * This is more efficient for document indexing.
 * @param {Array<{id: string, values: number[], metadata: object}>} vectors - Array of vector objects.
 */
async function upsertVectors(vectors) {
  try {
    // Pinecone handles batching internally, but we can chunk them if needed.
    // For now, we'll send them as a single batch.
