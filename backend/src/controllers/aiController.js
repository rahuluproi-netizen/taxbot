...
exports.uploadKnowledge = async (req, res) => {
  try {
    ...
    // 3. Generate Embeddings & Upsert to Pinecone (Batched)
    // Optimization: Batch embedding calls and vector upserts to reduce network roundtrips from 2N to 2*(ceil(N/100))
    if (chunks.length > 0) {
