...
exports.uploadKnowledge = async (req, res) => {
  try {
    ...
    // 1. Parse PDF - use async fs.promises for non-blocking I/O
    ...
    const timestamp = Date.now(); // Hoisted timestamp for consistency
    for (let i = 0; i < chunks.length; i += BATCH_SIZE) {
        ...
        // Upsert the entire batch to Pinecone in one request
    ...
    // Cleanup temp file - use async fs.promises
