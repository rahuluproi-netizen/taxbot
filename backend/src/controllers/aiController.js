...
exports.uploadKnowledge = async (req, res) => {
  try {
    ...
    if (chunks.length > 0) {
      // Performance optimization: Generate all embeddings in batch calls
      // and upsert vectors to Pinecone in batch, reducing network roundtrips from O(N) to O(N/100).
