...
exports.uploadKnowledge = async (req, res) => {
  try {
    ...
    // Bolt Optimization: Replaced sequential O(N) calls with batched calls for ~5-10x faster indexing
    const embeddings = await generateBatchEmbeddings(chunks);

    const vectorObjects = chunks.map((chunk, i) => ({
    ...
    await upsertVectors(vectorObjects);
