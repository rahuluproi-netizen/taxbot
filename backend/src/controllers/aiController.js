...
exports.uploadKnowledge = async (req, res) => {
  try {
    ...
    // Gemini's batchEmbedContents limit is 100 requests per call.
    const BATCH_SIZE = 100;
    for (let i = 0; i < chunks.length; i += BATCH_SIZE) {
        const batchChunks = chunks.slice(i, i + BATCH_SIZE);
        
        // Performance: Parallelize embedding generation for the batch
        ...
        // Performance: Batch upsert to Pinecone reduces network round-trips
