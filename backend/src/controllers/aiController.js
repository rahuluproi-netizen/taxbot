...
exports.uploadKnowledge = async (req, res) => {
  try {
    ...
    // 1. Parse PDF using async fs.promises
    ...
    // Optimized: Using batched calls to Gemini and Pinecone (batch size 100)
    const BATCH_SIZE = 100;
    const timestamp = Date.now(); // Hoist timestamp for consistency

    for (let i = 0; i < chunks.length; i += BATCH_SIZE) {
        const currentBatchChunks = chunks.slice(i, i + BATCH_SIZE);

        // Generate all embeddings for the current batch in one API call
        const embeddings = await generateEmbeddings(currentBatchChunks);
        
        // Prepare vectors for Pinecone batch upsert
        const vectors = currentBatchChunks.map((chunk, index) => ({
    ...
    // Cleanup temp file using async fs.promises
