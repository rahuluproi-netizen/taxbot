...
exports.uploadKnowledge = async (req, res) => {
  try {
    ...
    // 3. Generate Embeddings in batch & Upsert vectors to Pinecone
    // Optimization (⚡ Bolt): Use batch embedding API and batch vector upserts to replace sequential 1-by-1 API roundtrips
    ...
    if (upsertVectors) {
      ...
      for (const vector of vectors) {
        await upsertVector(vector.id, vector.values, vector.metadata);
      }
    }
