...
exports.uploadKnowledge = async (req, res) => {
  try {
    ...
    // Gemini supports up to 100 per batch. We'll use 100 as the batch size.
    const batchSize = 100;
    const timestamp = Date.now();
    for (let i = 0; i < chunks.length; i += batchSize) {
        const batch = chunks.slice(i, i + batchSize);
        
        // Generate embeddings for the whole batch in one call
        ...
        // Upsert the whole batch to Pinecone in one call
