...
exports.uploadKnowledge = async (req, res) => {
  try {
    ...
    // 3. Bolt Optimization: Batch generate embeddings and batch upsert vectors
    // Replaces O(N) sequential API calls with O(N/100) batched calls, dramatically speeding up document indexing.
    if (chunks.length > 0) {
