...
exports.uploadKnowledge = async (req, res) => {
  try {
    ...
    // 3. Generate Embeddings & Upsert to Pinecone (Batched for performance)
    ...
    for (let i = 0; i < chunks.length; i += BATCH_SIZE) {
        ...
        const vectors = batchChunks.map((chunk, index) => {
            ...
            return {
                id: `${req.user.id}_${timestamp}_${chunkIndex}`,
    ...
    // Cleanup temp file - Use async fs
