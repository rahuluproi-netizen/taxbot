...
exports.uploadKnowledge = async (req, res) => {
  try {
    ...
    // 1. Parse PDF using fs.promises for non-blocking I/O
    ...
    // BATCH_SIZE is 100 as per Gemini batch limit
    const BATCH_SIZE = 100;
    const timestamp = Date.now(); // Hoisted for efficiency and consistency

    for (let i = 0; i < chunks.length; i += BATCH_SIZE) {
        const batchChunks = chunks.slice(i, i + BATCH_SIZE);

        // ⚡ Bolt: Batch generate embeddings to reduce network roundtrips
        const batchEmbeddings = await generateBatchEmbeddings(batchChunks);
        
        const vectors = batchEmbeddings.map((embedding, index) => {
            const chunkIdx = i + index;
            return {
                id: `${req.user.id}_${timestamp}_${chunkIdx}`,
                values: embedding,
                metadata: {
        ...
        // ⚡ Bolt: Batch upsert to Pinecone for faster indexing
    ...
    // Cleanup temp file using fs.promises
