...
// Initialize Gemini chat model at module level to avoid redundant object instantiation
...
exports.uploadKnowledge = async (req, res) => {
  try {
    ...
    // This reduces network roundtrips from O(N) to O(N/100) by generating all embeddings and upserting in batch
...
exports.askQuery = async (req, res) => {
  try {
    ...
    // 3. Generate Answer with Gemini (uses the module-level model)
