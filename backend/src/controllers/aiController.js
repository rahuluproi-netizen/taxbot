...
exports.uploadKnowledge = async (req, res) => {
  try {
    ...
    // Optimization: Batch embeddings and upserts to reduce network roundtrips from O(N) to O(N/100)
    const BATCH_SIZE = 100;
    for (let i = 0; i < chunks.length; i += BATCH_SIZE) {
      const currentBatch = chunks.slice(i, i + BATCH_SIZE);
      const embeddings = await generateBatchEmbeddings(currentBatch);

      const vectors = currentBatch.map((chunk, index) => ({
