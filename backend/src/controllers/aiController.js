...
// Initialize Gemini LLM at the module level to prevent redundant instantiation on each request
...
exports.uploadKnowledge = async (req, res) => {
  try {
    ...
    // 3. Generate Embeddings & Upsert to Pinecone in optimized concurrent batches
    // Reduces sequential network requests from O(N) to O(N/100)
...
exports.askQuery = async (req, res) => {
  try {
    ...
    // 3. Generate Answer with Gemini (uses the module-level pre-initialized model)
