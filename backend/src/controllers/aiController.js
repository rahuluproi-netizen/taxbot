...
exports.uploadKnowledge = async (req, res) => {
  try {
    ...
    // 3. Generate Embeddings in Batches & Upsert to Pinecone
    // Optimization: Using batch APIs reduces network roundtrips and significantly speeds up processing.
    const BATCH_SIZE = 100;
    for (let i = 0; i < chunks.length; i += BATCH_SIZE) {
        const batchChunks = chunks.slice(i, i + BATCH_SIZE);
        const embeddings = await generateBatchEmbeddings(batchChunks);
        
        const vectors = batchChunks.map((chunk, indexInBatch) => ({
            id: `${req.user.id}_${Date.now()}_${i + indexInBatch}`,
