...
// Initialize chat model once at module level to avoid redundant instantiation on every request
...
exports.uploadKnowledge = async (req, res) => {
  try {
    ...
    // 3. Generate Embeddings & Upsert to Pinecone (Batched to reduce roundtrips from O(N) to O(N/100))
    if (chunks.length > 0) {
