...
exports.uploadKnowledge = async (req, res) => {
  try {
    ...
    if (chunks.length > 0) {
      // 3. Generate Batch Embeddings & Upsert to Pinecone in bulk
      // Optimization: Replaced sequential single-item embedding & vector upsert calls with batched requests.
