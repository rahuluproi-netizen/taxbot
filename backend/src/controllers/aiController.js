...
// Initialize Gemini models at the module level to prevent redundant object instantiation on each request
...
exports.uploadKnowledge = async (req, res) => {
  try {
    ...
    // 3. Generate Embeddings in a single batched call and Upsert in a single batched call
    if (chunks.length > 0) {
      const embeddings = await generateBatchEmbeddings(chunks);
      const baseTime = Date.now();

      const vectors = chunks.map((chunk, i) => ({
        id: `${req.user.id}_${baseTime}_${i}`,
        values: embeddings[i],
        metadata: {
          text: chunk,
          caId: req.user.id,
