...
export default function ChatWidget() {
  ...
  return (
    <div style={{ position: 'fixed', bottom: '2rem', right: '2rem', zIndex: 1000, fontFamily: 'inherit' }}>
      ...
      {!isOpen && (
        <button 
          ref={toggleRef} onClick={() => setIsOpen(true)} aria-label="Open chat assistant"
          aria-expanded="false" aria-controls="chat-window"
          style={{
      ...
      {isOpen && (
        <div
          id="chat-window" role="dialog" aria-label="Chat assistant" aria-modal="false"
        ...
        >
          ...
          <div style={{ 
            ...
            <button onClick={() => setIsOpen(false)} aria-label="Close chat" style={{ background: 'none', border: 'none', color: '#fff', cursor: 'pointer', fontSize: '1.2rem' }}>✕</button>
          </div>
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '1rem', display: 'flex', flexDirection: 'column', gap: '1rem', background: 'var(--background)' }}>
            ...
            {loading && <div role="status" aria-live="polite" style={{ fontSize: '0.8rem', color: 'var(--text-secondary)' }}>Assistant is typing...</div>}
          ...
          <div style={{ padding: '1rem', background: 'var(--surface)', borderTop: '1px solid var(--border)', display: 'flex', gap: '0.5rem' }}>
            ...
            />
            <button onClick={handleSend} aria-label="Send message" style={{ background: 'var(--primary)', color: '#fff', border: 'none', borderRadius: '50%', width: '40px', height: '40px', cursor: 'pointer' }}>➤</button>
          </div>
        </div>
      )}
    </div>
