...
export default function ChatWidget() {
  ...
  useEffect(() => {
    ...
    (isOpen ? inputRef : toggleRef).current?.focus();
  }, [isOpen]);
    ...
    <div style={{ position: 'fixed', bottom: '2rem', right: '2rem', zIndex: 1000, fontFamily: 'inherit' }}>
      ...
      {!isOpen && (
        <button 
          ...
          aria-label="Open AI Assistant" aria-expanded="false" aria-controls="chat-window"
          style={{
      ...
      {isOpen && (
        <div
          id="chat-window" className="glass-panel" role="dialog" aria-modal="false" aria-label="TaxBot AI Assistant"
          style={{
        ...
        >
          ...
          <div style={{ 
            ...
            <button 
              onClick={() => setIsOpen(false)} aria-label="Close AI Assistant"
          ...
          <div style={{ padding: '1rem', background: 'var(--surface)', borderTop: '1px solid var(--border)', display: 'flex', gap: '0.5rem' }}>
            <input 
              ...
              placeholder="Type your query..." aria-label="Chat input"
