...
export default function CaPanel() {
  ...
  const fetchDashboardData = async (userId: string) => {
    ...
    const typedTickets: Ticket[] = (ticketData || []) as unknown as Ticket[];
    ...
    setStats({
      ...
      tickets:
        typedTickets.filter((t: Ticket) => t.status === "Open").length || 0,
      solved:
        typedTickets.filter((t: Ticket) => t.status === "Resolved").length || 0,
    });
  };
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (res.ok) {
        setUploadMessage(
          `Success! Indexed ${result.chunks} text segments from the document.`,
        );
      } else {
      ...
      <main style={{ flex: 1, padding: "3rem" }}>
        ...
        >
          ...
          <p style={{ color: "var(--text-secondary)", marginBottom: "1.5rem" }}>
            Upload tax guides, FAQs, or compliance PDFs. Our AI will use these
            to answer your clients accurately.
          </p>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "1.5rem" }}>
            ...
            {uploadMessage && (
              <span
                style={{
                  color: uploadMessage.startsWith("Error")
                    ? "var(--error)"
                    : "var(--success)",
                  fontWeight: 500,
                }}
                ...
                >
                  ...
                  <td style={{ padding: "1rem" }}>
                    <span
                      style={{
                        background:
                          ticket.status === "Open" ? "#fee2e2" : "#dcfce7",
